...
public class ToDateConstruct extends Construct {
    ...
    private MetaExpression convert(MetaExpression source) {
        if (source.hasMeta(Date.class)) {
            ...
            result.storeMeta(new TemporalMetadataExpression(temporalConversionService.toDate(source.getMeta(Date.class))));
            return result;
        } else {
